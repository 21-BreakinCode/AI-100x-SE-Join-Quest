...
- Follow previous structure like [order.feature](../features/order.feature) with corresponding [steps script](../features/steps/order_steps.py) and implement below feature with new steps script due to this is entire new product just within a same project codebase.
- For your reference, you can create some files structure like: `features/steps/chess_steps.py` with corresponding feature files: `features/chess.feature`. then start to implement the logic under `src/chess_product/` just to separate the product scope.

## chess feature file bdd case
